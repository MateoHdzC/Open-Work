...
export class ModelDiscoveryEngine {
    ...
    try {
      if (provider.id === 'openai') {
          ...
          if (data.models && data.models.length > 0) {
            return data.models
              .map(m => {
                ...
                return this.inferModelInfo('google', cleanId, m.displayName);
              })
        ...
        if (res.ok) {
          ...
          if (data.data && data.data.length > 0) {
            return data.data.slice(0, 50).map(m => this.inferModelInfo('openrouter', m.id, m.name));
          }
        }
        ...
        if (res.ok) {
          const data = (await res.json()) as { data: Array<{ id: string }> };
          if (data.data && data.data.length > 0) {
            return data.data.map(m => this.inferModelInfo(provider.id, m.id));
          }
        }
      }
    } catch {}
  ...
  public inferModelInfo(providerId: string, modelId: string, displayName?: string): ModelInfo {
    ...
    const isComputer =
      clean.includes('astra') || clean.includes('computer') || clean.includes('sonnet') || clean.includes('gpt-4o');
    const isRecommended =
      clean.includes('gpt-4o') ||
      clean.includes('claude-3-7') ||
      clean.includes('gemini-2.5-flash') ||
      clean.includes('gemini-1.5-pro') ||
      clean.includes('deepseek-chat') ||
      clean.includes('astra');
